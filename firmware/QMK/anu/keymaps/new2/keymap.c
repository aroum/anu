...
#define COORDINATE_THRESHOLD_VALUE 20
...
uint16_t keycodeThresholdStart = 1600;
uint16_t delay = 20;
...
void axisToKeycodeComponent(uint8_t layer, uint8_t x_pin, uint8_t y_pin, uint16_t *keycodeThreshold, uint16_t *keycode)
{
...
void matrix_scan_user(void)
{
  ...
  if (layer >= 1)
  {
    ...
    if (x || y || (x * -1) || (y * -1) > COORDINATE_THRESHOLD_VALUE)
    {
